...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ErrorResponse> handleRecordNotFoundException(RecordNotFoundException exp) {
		ErrorResponse error = new ErrorResponse(exp.getMessage(), HttpStatus.NOT_FOUND.value(),
	...
	public ResponseEntity<ErrorResponse> handleEmailExistsException(EmailExistsException exp) {
		ErrorResponse error = new ErrorResponse(exp.getMessage(), HttpStatus.CONFLICT.value(),
				System.currentTimeMillis());
	...
	public ResponseEntity<ErrorResponse> handleMethodArgumentNotValidException(MethodArgumentNotValidException exp) {
		ErrorResponse error = new ErrorResponse(exp.getBindingResult().getFieldError().getDefaultMessage(),
	...
	public ResponseEntity<ErrorResponse> handleException(Exception exp) {
		ErrorResponse error = new ErrorResponse(exp.getMessage(), HttpStatus.BAD_REQUEST.value(),
				System.currentTimeMillis());
		return new ResponseEntity<ErrorResponse>(error, HttpStatus.BAD_REQUEST);
	}
